...
raw.metadata <- raw.metadata %>%
  dplyr::rename(nUMI = nCount_RNA,
                nGene = nFeature_RNA)
hr.merged@meta.data <- raw.metadata
head(hr.merged@meta.data)
...
```{r}
# remove empty droplets
hr.merged <- subset(hr.merged, subset = nUMI > 200)

# remove low quality cells
hr.merged <- subset(hr.merged, subset = nGene > 200)


# Extract timepoint prefix from cell names like "0hrs_AAACCCAAGCCATCCG-1"
timepoints <- sapply(strsplit(colnames(hr.merged), "_"), function(x) x[1])

# Assign as factor
hr.merged$timepoint <- factor(timepoints,
...
save(hr.merged, metadata, file = "hr.mergedASXL.droplets")
...
#SAVE AND LOAD
save(hr0f, hr12f, hr18f, hr24f, hr36f, hr3f, hr36f, hr6f, hr.merged.f, file="hr.meta.ASXL.f.RData")
